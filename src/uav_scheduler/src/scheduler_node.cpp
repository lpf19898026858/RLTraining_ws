                ...
                const std::map<std::string, geometry_msgs::Point>& poi_positions)
{
  ...
  while (!remaining.empty()) {
    ROS_INFO("========== [Scheduler] Starting assignment round %d ==========", round_counter);
    ROS_INFO("[Scheduler] %zu POIs remaining.", remaining.size());
    ...
    ROS_INFO("[Scheduler] Solver completed for round %d.", round_counter);
    ...
    // 检查是否分配了任何任务
    if (assigned.empty()) {
        ROS_WARN("[Scheduler] No new POIs were assigned in round %d. Exiting loop.", round_counter);
        break;
    }
    ...
    ROS_INFO("[Scheduler] Round %d finished. %zu POIs remaining.", round_counter, remaining.size());
    round_counter++;
  }
  ROS_INFO("========== [Scheduler] All assignment rounds finished ==========");
  // ==================【修改结束】==================
